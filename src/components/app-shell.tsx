...
function NavIcon({ kind }: { kind: string }) {
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">{kind === "home" ? <path d="m3 10 9-7 9 7v10H3V10Zm6 10v-7h6v7" /> : kind === "stock" ? <path d="M4 6h16M4 12h16M4 18h10" /> : kind === "context" ? <><rect x="4" y="4" width="16" height="16" rx="3" /><path d="M4 10h16M10 10v10" /></> : <path d="M4 4v16h16M8 16v-4m4 4V7m4 9v-6m4 6V4" />}</svg>;
}

export function AppShell({ accounts, children }: { accounts: Account[]; children: ReactNode }) {
  ...
  const stockPath = `/${buyer?.slug ?? "cliente-demo"}`;
  const links = [
    { href: "/", label: "Inicio", icon: "home" },
    { href: stockPath, label: "Stock y pedidos", icon: "stock" },
    { href: "/protocol", label: "Negociaciones", icon: "analytics" },
    { href: "/context", label: "Contexto", icon: "context" },
  ];
  const activeLabel = links.find((link) => link.href === pathname)?.label ?? "Tu espacio";
  ...
  return <div className="app-shell">
    ...
    <aside id="app-navigation" className={`app-sidebar ${menuOpen ? "is-open" : ""}`} onKeyDown={(event) => {
      ...
      <button ref={closeButton} type="button" className="icon-button mobile-nav-close" aria-label="Cerrar menú" onClick={closeMenu}>×</button>
      <Link href="/" className="hermes-brand" aria-label="Hermes, inicio"><span className="brand-mark"><HermesMark /></span><span>hermes</span><span className="brand-period">®</span></Link>
      <p className="nav-caption">TU ESPACIO</p>
      <nav className="app-nav" aria-label="Navegación principal">{links.map((link) => <Link href={link.href} key={link.href} className={pathname === link.href ? "active" : ""} aria-current={pathname === link.href ? "page" : undefined} onClick={() => setMenuOpen(false)}><NavIcon kind={link.icon} /><span>{link.label}</span><NavigationPending /></Link>)}</nav>
      <div className="sidebar-accounts"><p className="nav-caption">EMPRESAS</p>{accounts.map((account) => <Link key={account.id} href={`/${account.slug}`} onClick={() => setMenuOpen(false)} className={pathname === `/${account.slug}` ? "selected" : ""}><span className={`account-dot ${account.kind === "BUYER" ? "buyer" : `supplier-${account.slug === "distribuidora-norte" ? "cyan" : account.slug === "mayorista-andino" ? "amber" : "violet"}`}`} /><span>{account.legalName}</span><NavigationPending /></Link>)}</div>
      <div className="sidebar-foot"><HermesMark /><strong>Las mejores condiciones,<br />a una conversación.</strong><span>Entorno de demostración</span></div>
    </aside>
    <div className="app-main">
<header className="app-topbar"><button type="button" id="navigation-trigger" aria-controls="app-navigation" className="icon-button mobile-menu" aria-expanded={menuOpen} aria-label="Abrir navegación" onClick={() => setMenuOpen((value) => !value)}>☰</button><div className="breadcrumb">Tu espacio <span>/</span><strong>{activeLabel}</strong></div><div className="topbar-tools"><button className="theme-toggle" type="button" onClick={toggleTheme} aria-label="Cambiar entre modo claro y oscuro"><svg className="moon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 15.2A8.5 8.5 0 0 1 8.8 4a8.5 8.5 0 1 0 11.2 11.2Z" /></svg><svg className="sun" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="3.5" /><path d="M12 2v2m0 16v2M2 12h2m16 0h2M5 5l1.5 1.5m11 11L19 19M5 19l1.5-1.5m11-11L19 5" /></svg></button><div className="account-identity"><strong>{current?.legalName ?? "Hermes"}</strong><span>{current?.kind === "SUPPLIER" ? "PROVEEDOR" : "COMERCIO"}</span></div><span className="account-avatar">{initials}</span></div></header>
      <div id="main-content" className="app-content"><PurchaseFlowProvider>{children}</PurchaseFlowProvider></div>
